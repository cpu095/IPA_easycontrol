    ...
    def __call__(
        ...
        if sub_number > 0:
            ...
            for subject_image in subject_images:
                ...
                subject_image_np = subject_image.squeeze().cpu().numpy()  # 转换为 numpy 数组
                subject_image_np = subject_image_np.transpose(1, 2, 0)  # 转换维度顺序为 (H, W, C)
                plt.imshow(subject_image_np)
                plt.axis('off')  # 不显示坐标轴
                plt.savefig(f'subject_image_1_padded.png', bbox_inches='tight', pad_inches=0)
                plt.close()
