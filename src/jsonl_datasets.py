...
clip_image_processor = AutoProcessor.from_pretrained("/opt/liblibai-models/user-workspace/songyiren/FYP/sjc/IPA_easycontrol/siglip")
...
def make_train_dataset(args, tokenizer, accelerator=None):
    ...
    def preprocess_train(examples):
        ...
        if args.spacial_column is not None:
            spacial_images = [[load_image_safely(examples[column][i], args.cond_size) for column in spacial_columns] for i in range(len(examples[target_column]))]
        ...
        # Save target_images separately
        _examples["target_images"] = target_images  # Add target_images to the dictionary


        ##add clip
        # Process clip images for each sample
        clip_images = [clip_image_processor(images=Image.open(examples[args.target_column][i]).convert("RGB"), return_tensors="pt").pixel_values for i in range(len(examples[target_column]))]
        _examples["clip_images"] = clip_images  # Add clip_images to the dictionary
...
def collate_fn(examples):
    ...
    ## Stack target images (from the modified preprocess_train)
    target_images = [example["target_images"] for example in examples]
    ##
    ## Stack clip images (added section for clip_images)
    ...
    return {
        ...
        "clip_images": clip_images,  # Returning clip_images as part of the batch
    }